...
int main(int argc, char * argv[]) {
  ...
  ViewDrift* vedo = new ViewDrift(); ;
  aval->SetSensor(sensor);
  track->SetSensor(sensor);
  track->SetParticle("mu");
  track->SetMomentum(1.2e9);
  double xx=0;
  double ee=0.;
  int k=0;

  const int nEvents = 10;
  gas->ElectronVelocity(100,0,0,0,0,0,xx,ee,xx);
  std::cout<<"nelle mie condizioni la velocità di drift degli elettroni è " << ee << "\n";
  ...
  for (int i = 0; i < nEvents; ++i) {
    ...
    while (track->GetCluster(xc, yc, zc, tc, nc, ec, extra)) {
      esum += ec;
      nsum += nc;
      std::cout<<"Ho "<<nc<<" cluster, alla coordinata "<<xc<<" al tempo " <<tc<<"\n";
      hEnergy->SetPoint(k,tc,.1-xc);
      k++;
      //Loop over the electrons in the cluster
      // for(int h = nc; h--;)
      // 	{ track->GetElectron(h,xx,y0,z0,t0,ee,dx0,dy0,dz0);
      // 	  aval->AvalancheElectron(xx,y0,z0,t0,ee,dx0,dy0,dz0);
      // 	}
