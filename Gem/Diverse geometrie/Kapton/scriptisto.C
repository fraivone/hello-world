...
int main(int argc, char * argv[]) {
  ...
  TH1F* totalelectrons = new TH1F("totalelectrons","",10,20,120);
  TH1F* lostelectrons =  new TH1F("lostelectrons","",10,20,120);
  TH1F* fractionup =  new TH1F("fractionup","",10,20,120);
  TH1F* fractionplastic  =  new TH1F("fractionplastic","",10,20,120);
  TH1F* fractionlow =  new TH1F("fractionlow","",10,20,120);
  ...
  std::string search = "/od"; //stringa da <cercare 
	...
	a +="/";
	  ...
	  { a += entry->d_name;
	    if(a.find(search)!=std::string::npos)
	      {
		voltages=0;
		sscanf(entry->d_name,"%s %s %s",buff , buff,buff2);
    ...
    totalelectrons->SetXTitle("Kapton thickness [#mum]");
    totalelectrons->SetYTitle("Number of electrons");
    totalelectrons->SetStats(kFALSE);
    TCanvas* c4 = new TCanvas("c4", "totalelectrons",1920,1080);
    ...
    char c[200];
    char b[200];
    sprintf(c,"GEM dimension [#mum]   -  hole diam = %.0f-%.0f,  pitch = %.0f", middia*10000,outdia*10000,pitch*10000);
    TLatex *text = new TLatex(20, 131, c);
    sprintf(b,"#splitline{GEM Field: %.0f}{#splitline{Drift Field: %.0f kV/cm}{Transfer field: %.0f kV/cm}}",(lowmetal-upmetal)/5,(upmetal-drft)/100,(induction-lowmetal)/100);   //Fattore 100 ingloba spazio di drift e conversione V->kV
    TLatex *text2 = new TLatex(90,50, b);
