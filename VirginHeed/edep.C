...
int main(int argc, char * argv[]) {
  ...
  for (int i = 0; i < nEvents; ++i) {
    ...
    while (track->GetCluster(xc, yc, zc, tc, nc, ec, extra)) {
      esum += ec;
      std::cout << "Ho perso " << ec << " eV"<<"\n";
      std::cout << "Producendo " <<nc << " elettroni\n";
      if(nc > 1){
	for(int k = nc; k--;)
	  { track->GetElectron(k,x0,y0,z0,t0,ec,dx0,dy0,dz0);
	    std::cout<<"L'elettrone "<<k<<" aveva x: "<<x0<<", Energia :"<<std::fixed << std::setprecision(7)<<ec<<"\n\n";  }
